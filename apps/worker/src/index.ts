...
app.post('/auth/login', async c => { const input = authSchema.pick({email:true,password:true}).safeParse(await c.req.json()); if(!input.success) return json({error:'Invalid credentials'},400); const user = await c.env.DB.prepare('SELECT * FROM users WHERE email=?').bind(input.data.email.toLowerCase()).first<any>(); if(!user || user.status !== 'active') return json({error:'Invalid credentials'},401); const check = await hashPassword(input.data.password,user.password_salt); if(check.hash !== user.password_hash) return json({error:'Invalid credentials'},401); return json({user:safeUser(user),token:await issueUserToken(user,c.env.JWT_SECRET)}) })
